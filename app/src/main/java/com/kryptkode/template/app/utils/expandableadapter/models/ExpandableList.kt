...
class ExpandableList<T: ExpandableGroup<P, C>, P: Parent, C:Child>(var groups: List<T>) {
     ...
     * @return the index of a group within the [visibleItemCount]
     */
     ...
     * @return the index of a group within the [visibleItemCount] or 0 if the
     * groups.indexOf cannot find the group
     */
    fun getFlattenedGroupIndex(group: ExpandableGroup<P, C>?): Int {
    ...
    fun getFlattenedChildIndex(listPosition: ExpandableListPosition?): Int {
        val groupIndex = listPosition?.groupPos ?: 0
        val childIndex = listPosition?.childPos ?: 0
        var runningTotal = 0
        for (i in 0 until groupIndex) {
            runningTotal += numberOfVisibleItemsInGroup(i)
        }
     ...
     */
    fun getExpandableGroup(listPosition: ExpandableListPosition): T {
