...
public class TemplateView extends FrameLayout {
    ...
    private void applyStyles() {
        ...
        int ctaColor = styles.getCallToActionBackgroundColorInt();
        Drawable background = callToActionView.getBackground();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            background.setTint(ctaColor);
        } else {
             background = DrawableCompat.wrap(background);
            DrawableCompat.setTint(background.mutate(), ctaColor);
        }
        callToActionView.setBackground(background);
